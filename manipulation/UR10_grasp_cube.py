...
def main():
    ...
    CUBE_SIDE = 0.05                         # 5 cm cube
    can_z = TABLE_SURFACE_Z + CUBE_SIDE / 2  # half cube height above table
    ...
    # Drop the EE 3 cm further so the fingertips close around the cube
    # instead of hovering above it.
    goal_target = [can_x, can_y, can_z + GRASP_OFFSET_Z + 0.01 - 0.03]
    ...
    # ── Place a 5 cm cube on the table ─────────────────────────────
    half = CUBE_SIDE / 2
    ...
    cube_vis = p.createVisualShape(
        p.GEOM_BOX, halfExtents=[half, half, half],
        rgbaColor=[0.85, 0.25, 0.20, 1.0],   # tomato red, matches old demo
        physicsClientId=cid)
    can_id = p.createMultiBody(
        baseMass=0,                          # static (equivalent to useFixedBase=True)
    ...
    for link_idx in range(-1, n_joints_r):
        p.setCollisionFilterPair(env.robot_id, can_id, link_idx, -1, 0,
                                 physicsClientId=cid)
    print(f"[OBJ] Placed {int(CUBE_SIDE*100)} cm cube at "
    ...
    if path:
        ...
        with open("results/UR10_grasp_cube_world_state.txt", "w") as f:
            f.write("=" * 62 + "\n")
        ...
        print("[FILE] Saved results/UR10_grasp_cube_world_state.txt")

        with open("results/UR10_grasp_cube_path.txt", "w") as f:
            f.write("=" * 62 + "\n")
            ...
            f.write("-" * 62 + "\n")
        print("[FILE] Saved results/UR10_grasp_cube_path.txt")
        ...
        if _args.save_gif:
            ...
            _gif_path = os.path.join(
                ...
                'visualization', 'gifs',
                f'pybullet_UR10_grasp_cube_{_gif_tag}.gif')
            save_path_gif(

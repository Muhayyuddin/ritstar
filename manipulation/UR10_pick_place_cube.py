...
# 5 cm cube (replaces the YCB tomato soup can from the can demo)
CUBE_SIDE  = 0.05
CAN_HEIGHT = CUBE_SIDE   # kept as CAN_HEIGHT so downstream writes work
...
def attach_can_to_gripper(env, q_grasp):
    ...
    print(f"[CUBE] Spawning {int(CUBE_SIDE*100)} cm cube at "
          f"[{can_pos_w[0]:.3f}, {can_pos_w[1]:.3f}, {can_pos_w[2]:.3f}]")

    _half = CUBE_SIDE / 2
    _cube_col = p.createCollisionShape(
        p.GEOM_BOX, halfExtents=[_half, _half, _half],
        physicsClientId=cid)
    _cube_vis = p.createVisualShape(
        p.GEOM_BOX, halfExtents=[_half, _half, _half],
        rgbaColor=[0.85, 0.25, 0.20, 1.0],   # tomato red
        physicsClientId=cid)
    can_id = p.createMultiBody(
        baseMass=0.1,                         # non-zero so fixed constraint works
        baseCollisionShapeIndex=_cube_col,
        baseVisualShapeIndex=_cube_vis,
    ...
    if path:
        ...
        if _args.save_gif:
            ...
            _gif_path = os.path.join(
                ...
                'visualization', 'gifs',
                f'pybullet_UR10_pick_place_cube_{_gif_tag}.gif')
            save_path_gif(
        ...
        with open("results/UR10_pick_place_cube_world_state.txt", "w") as f:
            f.write("=" * 62 + "\n")
        ...
        print("[FILE] Saved results/UR10_pick_place_cube_world_state.txt")

        with open("results/UR10_pick_place_cube_path.txt", "w") as f:
            f.write("=" * 62 + "\n")
            ...
            f.write("-" * 62 + "\n")
        print("[FILE] Saved results/UR10_pick_place_cube_path.txt")
    else:

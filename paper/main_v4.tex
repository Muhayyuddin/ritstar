...
\caption{Benchmark Evaluations (10 trials, 200\,s timeout).
  ...
  $c^{\mathrm{med}}_{\mathrm{init}}$ / $c^{\mathrm{med}}_{\mathrm{final}}$. \textbf{Bold}: best per column group.
  All planners achieve 100\% success on all environments.}
...
\toprule
 ...
 & \multicolumn{4}{c|}{\textbf{6-D UR10 Drill ($\mathbb{R}^6$)}}
...
\midrule
RIT*
 & 0.0128 & 0.0197 & \textbf{0.9578} & 0.9135
 & \textbf{0.0213} & \textbf{0.0403} & 2.8167 & \textbf{2.1608}
 & \textbf{0.0601} & 0.0915 & \textbf{4.6868} & 1.4644
 & 0.8135 & 3.4255 & \textbf{16.5747} & \textbf{8.5778}
 & 100 \\
BIT*
 & 0.0183 & 0.0308 & 0.9698 & \textbf{0.9108}
 & 0.0476 & 0.0698 & \textbf{2.4923} & 2.2027
 & 0.0651 & \textbf{0.0820} & 5.2256 & 1.5415
 & \textbf{0.7657} & \textbf{0.8834} & 18.5069 & 10.7092
 & 100 \\
Inf.\ RRT*
 & 0.0129 & 0.0289 & 0.9946 & 0.9165
 & 0.0312 & 0.0464 & 2.7233 & 2.2278
 & 0.1390 & 0.2114 & \textbf{4.6868} & 1.6975
 & 1.5915 & 2.2198 & 22.9509 & 11.5773
 & 100 \\
AIT*
 & 0.0111 & 0.0217 & 1.0285 & 0.9116
 & 0.0236 & 0.0433 & 2.9939 & 2.2563
 & 0.0697 & 0.1159 & 4.7929 & 1.4588
 & 0.8070 & 1.4469 & 22.9509 & 11.0986
 & 100 \\
EIT*
 & \textbf{0.0106} & \textbf{0.0162} & 1.0285 & 0.9124
 & 0.0247 & 0.0413 & 2.9939 & 2.2449
 & 0.0705 & 0.1397 & 4.7929 & \textbf{1.4530}
 & 0.8095 & 1.4452 & 22.9509 & 11.4415
 & 100 \\
APT*
 & 0.0127 & 0.0189 & 0.9946 & 0.9225
 & 0.0312 & 0.0465 & 2.7233 & 2.4436
 & 0.1391 & 0.2126 & \textbf{4.6868} & 1.6836
 & 1.6003 & 2.2284 & 22.9509 & 14.0232
 & 100 \\
\bottomrule
\end{tabular}}
\end{table*}
...
\caption{Ablation study: mean path cost ($\pm$ std) over 10 trials;
  200\,iter for 2-D/3-D, 50\,iter for 6-D/14-D.
  \textbf{Bold}: best complete-run (10/10) practical variant per column
  (oracle row excluded).
...
\end{tabular}}
\vspace{1pt}
{\footnotesize $^\dagger$Oracle (ground-truth) metric; not available in practice.\quad
$^\ddagger$9/10 trials completed within the 350\,s limit; mean over 9 trials.}
\end{table}
...
$+8.7\%$ on 3-D Diagonal (2.388 vs.\ 2.196) where metric anisotropy is
strong, and $+6.8\%$ on 14-D Tiago (9.226 vs.\ 8.638).
On 2-D Random World the obstacle distribution is more isotropic, so the
cost difference (0.916 vs.\ 0.921) lies within one standard deviation and
is not statistically significant. The w/o\,Riem.\,samp.\ variant also
experienced a planning timeout (9/10 trials completed) on the 6-D Drill,
confirming instability without metric-guided sampling.
\emph{Cascading} has negligible effect on final cost across all
environments ($<0.1\%$ vs.\ full RIT*), confirming that L1/L2
filtering is a pure speed optimisation that does not sacrifice
solution quality.
\emph{CARM} (``w/o CARM'' uses the oracle metric directly): the
oracle consistently yields marginally lower cost since it provides
perfect metric information from iteration~1, whereas CARM must learn
the field online. The oracle advantage is less than $1\%$ in all
environments ($-0.9\%$ on 2-D, $-1.0\%$ on 3-D Diagonal, $-0.3\%$
on 6-D Drill, $-0.6\%$ on 14-D Tiago), demonstrating that CARM
closely approximates the oracle metric across the full dimensionality
range tested.
\emph{Smoothing} is the most critical component at high dimensions:
removing it degrades 14-D Tiago cost by $+8.8\%$ (9.396 vs.\ 8.638)
and 6-D Drill by $+3.2\%$ (1.537 vs.\ 1.489), reflecting the highly
non-geodesic first feasible paths returned by the tree before
post-processing.
...
CARM overhead is below $1\%$ across all environments tested:
$+0.9\%$ in 2-D, $+1.0\%$ in 3-D, $+0.3\%$ in 6-D, and $+0.6\%$
in 14-D.
The consistently low overhead confirms that CARM closely approximates
the oracle metric at all dimensionalities, and that its online learning
converges rapidly from collision feedback.
Critically, at the highest dimension tested (14-D Tiago bimanual) the
CARM overhead is only $+0.6\%$, demonstrating that the learned metric

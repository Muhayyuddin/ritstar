...
\For{$t = 1, \ldots, T$}{ \nllabel{line:loop}
  ...
  \If{$t \bmod \Delta = 0$}{ \nllabel{line:carmcheck}
    \textsc{CARM\_Update}$(\text{collision points}, G)$\; \nllabel{line:carmupdate}
  }
...
\caption{CARM metric update (every $\Delta$ iterations)}
\label{alg:carm}
\end{algorithm}
...
Under \cref{assum:bounded}, RIT* is asymptotically optimal:
the cost of its solution converges almost surely to the optimum
$c^*$ as the number of samples $n \to \infty$.
\end{theorem}

\begin{proof}[Proof sketch]
The connection radius~\eqref{eq:radius} satisfies the conditions
of~\cite{Karaman2011} under the metric-induced measure:
specifically, $r_n^R \ge \gamma_R (\log n / n)^{1/d}$ with
$\gamma_R > (2/d)^{1/d}(\muR(\IR)/\zeta_d)^{1/d}$, which ensures
$k$-connectivity of the Riemannian proximity graph.
The containment $\IR \subseteq \IE$ guarantees that informed sampling
in~$\IR$ preserves completeness.
When CARM is active, $s(x) \ge 1$ ensures
$\IR^{\mathrm{CARM}} \subseteq \IE$, preserving completeness.
The proof otherwise follows BIT*~\cite{Gammell2020} with $\muR$
replacing the Euclidean measure.
\end{proof}
...
\textbf{Environments.}
We evaluate on nine environments spanning three dimensionalities:
4$\times$2-D (Random World, Maze, Narrow Passage, Bug Trap),
...
(2)~In 2-D, RIT* achieves the lowest cost on Random World ($p < 0.01$)
and Narrow ($p < 0.01$), and
...
(4)~On planning time, RIT* is the fastest on 7 of 9~environments,
including a $44\times$ speedup on 2-D Random World versus APT*
and a $13\times$ speedup on 2-D Maze versus BIT*.

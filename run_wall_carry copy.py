...
def main():
    ...
    place_target = [can_x + 0.02, WALL_Y + side_clearance - 0.13, grasp_z]
    ...
    if path:
        ...
        with open("results/wall_carry_world_state.txt", "w") as f:
            ...
            f.write("--- Start (grasp, −y side) ---\n")
            f.write(f"  q  : [{', '.join(f'{v:.6f}' for v in q_start)}]\n")
            f.write(f"  EE : [{pos_s[0]:.5f}, {pos_s[1]:.5f}, "
                    f"{pos_s[2]:.5f}]\n\n")
            f.write("--- Goal (+y side, place) ---\n")
            f.write(f"  q  : [{', '.join(f'{v:.6f}' for v in q_goal)}]\n")
            f.write(f"  EE : [{pos_g[0]:.5f}, {pos_g[1]:.5f}, "
                    f"{pos_g[2]:.5f}]\n\n")
            f.write(f"--- Path ---\n")

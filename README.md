...
<p align="center">
  <img src="assets/gifs/config_2d_random_world_tree_growth_carm.gif" width="45%" alt="RIT* tree growth (2-D random world, CARM enabled)"/>
  <img src="assets/images/path_comparison_2d_random_world.png" width="45%" alt="Path comparison across planners (2-D random world)"/>
</p>
...
* **Table II (benchmark).** With the default [`config/run_config.yaml`](config/run_config.yaml):

  ```bash
  python run_from_config.py
  ```

  Runs the seven planners
  (RIT\*, GA-RRT\*, BIT\*, Informed RRT\*, AIT\*, EIT\*, APT\*)
  on the four paper environments
  (2D Random World, 3D Diagonal, UR10 pick-and-place, Tiago 14D).
...
* **Table III (ablation).**
  ...
  Runs five RIT\* variants (full, no Riemannian sampling, no cascading, no
  CARM, no smoothing) × 4 environments × 10 trials, and writes
  `results/ablation.csv` (per-trial data),
  `results/ablation_summary.csv` (mean ± std), and a paste-ready LaTeX table
